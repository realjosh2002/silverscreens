...
const profileMenuLinks = [
  ...
  { icon: '💎', label: 'Subscription', href: '/subscription' },
        ...
        <aside style={{ width: SB_W, flexShrink: 0, background: '#0B0F14', borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column' as const, overflowY: 'auto' as const, overflowX: 'hidden', transition: 'width 0.2s ease' }}>
            ...
            {mainMenuLinks.map(l => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: sidebarOpen ? 10 : 0 }}>
                  ...
                  {sidebarOpen && <span style={{ fontFamily: M, fontSize: 15, color: '#A8B0BD' }}>{l.label}</span>}
                </div>
                {sidebarOpen && l.badge > 0 && <span style={{ background: RED, color: '#fff', borderRadius: 12, fontSize: 13, fontWeight: 700, padding: '1px 6px', minWidth: 18, textAlign: 'center' as const }}>{l.badge}</span>}
              </div>
            ))}
          </nav>
          {/* Upgrade box */}
          {sidebarOpen && (
            <div style={{ margin: '10px 10px 14px', borderRadius: 10, background: 'linear-gradient(135deg, #1a0507, #2a0b0e)', border: '1px solid rgba(200,32,42,0.25)', padding: '14px 12px', textAlign: 'center' as const, flexShrink: 0 }}>
              ...
              <div style={{ fontFamily: B, fontSize: 15, color: '#F5F5F5', marginBottom: 4 }}>Upgrade to Pro</div>
              <div style={{ fontFamily: M, fontSize: 13, color: '#6A7080', marginBottom: 10, lineHeight: 1.5 }}>Unlock advanced features and get more visibility.</div>
              <button onClick={() => router.push('/pricing')} style={{ width: '100%', padding: '8px', background: RED, border: 'none', borderRadius: 6, color: '#fff', fontFamily: B, fontSize: 14, letterSpacing: 1, cursor: 'pointer' }}>UPGRADE NOW</button>
            </div>
          )}
        </aside>
