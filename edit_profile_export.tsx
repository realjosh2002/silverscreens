...
// â”€â”€ Departments & Roles (from lib/roles.ts â€” inlined for self-containment) â”€â”€
...
// â”€â”€ Custom dark-themed dropdown â€” replaces native <select> â”€â”€
function Select({
...
// â”€â”€ Department + Role selector â€” Option D chip layout â”€â”€
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative' as const, marginBottom: 12 }}>
        <span style={{ position: 'absolute' as const, left: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 14, color: '#6A7080' }}>ðŸ”</span>
        <input
          type="text"
      ...
      {totalRoles >= MAX_ROLES && (
        <div style={{ marginBottom: 10, padding: '7px 12px', background: 'rgba(200,32,42,0.08)', border: '1px solid rgba(200,32,42,0.2)', borderRadius: 6, fontFamily: M, fontSize: 14, color: RED }}>
          âš ï¸ Maximum {MAX_ROLES} roles reached. Remove a role below to add another.
        </div>
      )}
            ...
            {selectedRoles.map(r => (
              <span key={`${r.department}-${r.role}`} style={{
                ...
                <span onClick={() => removeRole(r.department, r.role)}
                  style={{ cursor: 'pointer', fontSize: 15, lineHeight: 1, opacity: 0.6 }}>Ã—</span>
              </span>
            ))}
          </div>
        </div>
...
const profileMenuLinks = [
  { icon: 'ðŸ‘¤', label: 'My Profile', href: '/my-profile' },
  { icon: 'âš™ï¸', label: 'Settings', href: '/settings' },
  { icon: 'ðŸšª', label: 'Logout', href: '/login' },
]
...
export default function CreateProfilePage() {
  ...
  useEffect(() => {
    try {
      ...
      // Edit mode â€” auto-restore without prompt
      if (draft?.editMode) {
  	setIsEditMode(true)
  	applyDraft(draft)
	}
  ...
  useEffect(() => {
	...
	const data = rawData
      ...
      console.log('[DEBUG] p.first_name:', p?.first_name)
	console.log('[DEBUG] p.aspirant_media:', JSON.stringify(p?.aspirant_media))
  ...
  // Matches the API's calculateCompletion â€” same fields & weights
  const completion = (() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' as const, height: '100vh', overflow: 'hidden', background: '#050505', fontFamily: M, color: '#F5F5F5' }}>
      ...
      {/* â•â•â• FULL-WIDTH HEADER â€” logo far left, user right (matches dashboard) â•â•â• */}
      <div style={{ height: 60, flexShrink: 0, background: '#0B0F14', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', padding: '0 24px', gap: 16, zIndex: 50 }}>
        ...
        <div style={{ position: 'relative' as const }}>
          <div onClick={() => setShowDropdown(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '6px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <span style={{ color: '#6A7080', fontSize: 14 }}>â–¼</span>
          </div>
          {showDropdown && (
            <>
      ...
      {/* â•â•â• BODY â€” sidebar + content â•â•â• */}
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>

        {/* â•â•â• STANDARD SIDEBAR â€” matches all other aspirant pages â•â•â• */}
          ...
          <div style={{ display: 'flex', gap: 20, padding: '20px 24px', flex: 1, overflowY: 'auto' as const }}>
            ...
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: M, fontSize: 15, color: '#6A7080', marginTop: 3 }}>{isEditMode ? 'Update your details â€” changes will be sent for admin review' : 'Complete your profile to start applying for castings'}</div>
              </div>
              ...
              {isEditMode && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'rgba(212,166,74,0.07)', border: '1px solid rgba(212,166,74,0.25)', borderRadius: 10 }}>
                  <span style={{ fontSize: 18, flexShrink: 0 }}>âœï¸</span>
                  <div>
              ...
              {/* Draft prompt â€” shown when a previous draft is detected */}
              {showDraftPrompt && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 18px', background: 'rgba(212,166,74,0.07)', border: '1px solid rgba(212,166,74,0.25)', borderRadius: 10, flexWrap: 'wrap' as const }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
                    ...
                    <button onClick={continueDraft} style={{ padding: '8px 20px', background: GOLD, border: 'none', borderRadius: 6, color: '#000', fontFamily: B, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>Continue Draft â†’</button>
                  </div>
                </div>
              )}
              ...
              {draftRestored && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 8 }}>
                  ...
                  <span style={{ fontFamily: M, fontSize: 14, color: '#86efac' }}>Draft restored â€” pick up right where you left off.</span>
                </div>
              )}
              ...
              {activeSection === 1 && (
                <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '20px' }}>
                  <SecHead num={1} icon="ðŸ‘¤" title="Basic Information" sub="Personal details for your profile" />
              ...
              {/* â”€â”€ SECTION 2: Physical Details â”€â”€ */}
              {activeSection === 2 && (
                <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '20px' }}>
                  <SecHead num={2} icon="ðŸ“" title="Physical Details" sub="Your physical measurements and appearance" />
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
              ...
              {/* â”€â”€ SECTION 3: Departments & Roles â”€â”€ */}
              {activeSection === 3 && (
                <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '20px' }}>
                  <SecHead num={3} icon="ðŸŽ¬" title="Departments & Roles" sub="Select the departments and roles that best represent your skills (max 5 roles)" />
                    ...
                    {credits.map((c, i) => (
                      <div key={i} style={{ marginBottom: 10, padding: 14, background: 'rgba(255,255,255,0.02)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, alignItems: 'flex-end' }}>
                          ...
                          <button onClick={() => removeCredit(i)} style={{ background: 'rgba(200,32,42,0.15)', border: '1px solid rgba(200,32,42,0.3)', borderRadius: 6, padding: '8px 10px', color: RED, cursor: 'pointer', fontFamily: M, fontSize: 14, marginBottom: 2 }}>âœ• Remove</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {activeSection === 4 && (
                <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '20px' }}>
                  <SecHead num={4} icon="ðŸ–¼ï¸" title="Media & Portfolio" sub="Add photos, videos and showreels to highlight your talent" />
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        ...
                        {form.profilePhoto && <span style={{ color: '#22c55e', fontSize: 16 }}>âœ“</span>}
                      </div>
                      <div onClick={() => profilePhotoRef.current?.click()} style={{ height: 130, borderRadius: 6, overflow: 'hidden', background: 'rgba(255,255,255,0.03)', border: `2px dashed ${form.profilePhoto ? '#22c55e' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                        {form.profilePhoto ? <img src={form.profilePhoto} style={{ width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none' }} alt="" /> : <div style={{ textAlign: 'center' as const, color: '#6A7080' }}><div style={{ fontSize: 28, marginBottom: 6 }}>ðŸ“·</div><div style={{ fontFamily: M, fontSize: 14 }}>Click to upload</div><div style={{ fontFamily: M, fontSize: 14, marginTop: 2 }}>JPG, PNG up to 5MB</div></div>}
                      </div>
                      {form.profilePhoto && (
                        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                          <button onClick={() => profilePhotoRef.current?.click()} style={{ flex: 1, padding: '6px 0', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 5, color: '#A8B0BD', fontFamily: M, fontSize: 13, cursor: 'pointer' }}>ðŸ”„ Replace</button>
                          <button onClick={(e) => { e.stopPropagation(); setForm(p => ({ ...p, profilePhoto: '' })) }} style={{ flex: 1, padding: '6px 0', background: 'rgba(200,32,42,0.1)', border: '1px solid rgba(200,32,42,0.25)', borderRadius: 5, color: RED, fontFamily: M, fontSize: 13, cursor: 'pointer' }}>âœ• Remove</button>
                        </div>
                      )}
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 16 }}>
                      ...
                      <div style={{ fontFamily: M, fontSize: 14, color: '#6A7080', marginBottom: 8 }}>Add at least 3 photos â€” no watermarks or contact numbers</div>
                      <div style={{ background: 'rgba(212,166,74,0.06)', border: '1px solid rgba(212,166,74,0.2)', borderRadius: 6, padding: '8px 12px', marginBottom: 10 }}>
                        <div style={{ fontFamily: M, fontSize: 13, color: GOLD, fontWeight: 700, marginBottom: 6 }}>ðŸ’¡ Recommended photo types (up to 10)</div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 12px' }}>
                          {['Headshot', 'Mid Shot', 'Full Length', 'Traditional', 'Western', 'Ethnic', 'Casual', 'Formal', 'Smile', 'Serious'].map((cat, idx) => (
                            <div key={cat} style={{ fontFamily: M, fontSize: 13, color: 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center', gap: 6, padding: '1px 0' }}>
                              ...
                              {galleryPhotos[idx] && <span style={{ color: '#22c55e', fontSize: 12, marginLeft: 'auto' }}>âœ“</span>}
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div onClick={() => galleryRef.current?.click()} style={{ height: 44, borderRadius: 6, background: 'rgba(255,255,255,0.03)', border: '2px dashed rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: 8 }}>
                        <span style={{ fontSize: 18 }}>âž•</span><span style={{ fontFamily: M, fontSize: 14, color: '#6A7080' }}>Add photos ({galleryPhotos.length}/10)</span>
                      </div>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        ...
                        {showreelUrl && <span style={{ color: '#22c55e', fontSize: 14 }}>âœ“</span>}
                      </div>
                      ...
                      <div onClick={() => showreelRef.current?.click()} style={{ height: 60, borderRadius: 6, background: showreelUrl ? 'rgba(34,197,94,0.06)' : 'rgba(255,255,255,0.03)', border: `2px dashed ${showreelUrl ? '#22c55e' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: 8 }}>
                        ...
                        <span style={{ fontFamily: M, fontSize: 14, color: showreelUrl ? '#22c55e' : '#6A7080' }}>{showreelUrl ? 'âœ“ Showreel added â€” click to replace' : 'Upload MP4 file'}</span>
                      </div>
                      {showreelUrl && (
                        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                          <button onClick={() => showreelRef.current?.click()} style={{ flex: 1, padding: '6px 0', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 5, color: '#A8B0BD', fontFamily: M, fontSize: 13, cursor: 'pointer' }}>ðŸ”„ Replace</button>
                          <button onClick={(e) => { e.stopPropagation(); setShowreelUrl('') }} style={{ flex: 1, padding: '6px 0', background: 'rgba(200,32,42,0.1)', border: '1px solid rgba(200,32,42,0.25)', borderRadius: 5, color: RED, fontFamily: M, fontSize: 13, cursor: 'pointer' }}>âœ• Remove</button>
                        </div>
                      )}
                          ...
                          {otherVideos.map((_, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: 4, marginBottom: 4 }}>
                              ...
                              <button
                                ...
                                style={{ padding: '3px 10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 4, color: '#A8B0BD', fontFamily: M, fontSize: 12, cursor: 'pointer' }}>ðŸ”„ Replace</button>
                              <button
                                onClick={() => setOtherVideos(p => p.filter((_, idx) => idx !== i))}
                                style={{ padding: '3px 10px', background: 'rgba(200,32,42,0.1)', border: '1px solid rgba(200,32,42,0.25)', borderRadius: 4, color: RED, fontFamily: M, fontSize: 12, cursor: 'pointer' }}>âœ• Remove</button>
                            </div>
                          ))}
                        </div>
                      )}
              ...
              {/* â”€â”€ SECTION 5: Review â”€â”€ */}
              {activeSection === 5 && (
                <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '20px' }}>
                  <SecHead num={5} icon="âœ…" title="Review & Submit" sub="Review your profile before publishing" />
                  ...
                  {form.profilePhoto && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, padding: '12px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6 }}>
                      ...
                      <div>
                        <div style={{ fontFamily: M, fontSize: 14, fontWeight: 700, color: '#F5F5F5' }}>{`${form.title} ${form.firstName} ${form.lastName}`.trim() || 'â€”'}</div>
                        <div style={{ fontFamily: M, fontSize: 14, color: '#A8B0BD', marginTop: 2 }}>Aspirant â€¢ {form.gender}</div>
                      </div>
                      <span style={{ color: '#22c55e', fontSize: 18, marginLeft: 'auto' }}>âœ“ Photo Added</span>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                    {[
                      ['Full Name',     `${form.title} ${form.firstName} ${form.lastName}`.trim() || 'â€”'],
                      ['Email',         form.email    || 'â€”'],
                      ['Mobile',        form.mobile   || 'â€”'],
                      ['Gender',        form.gender],
                      ['Date of Birth', form.dob      || 'â€”'],
                  ...
                  <div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, marginBottom: 16 }}>
                    {selectedRoles.length === 0 ? (
                      <span style={{ fontFamily: M, fontSize: 14, color: '#6A7080' }}>â€” No roles selected</span>
                    ) : (
                  ...
                  <div style={{ padding: '9px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, marginBottom: 16 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 4 }}>
                      ...
                      )) : <span style={{ fontFamily: M, fontSize: 14, color: '#6A7080' }}>â€”</span>}
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '9px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, marginBottom: 16 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 4 }}>
                      ...
                      )) : <span style={{ fontFamily: M, fontSize: 14, color: '#6A7080' }}>â€” No skills selected</span>}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8, marginBottom: 16 }}>
                    {credits.map((c, i) => (
                      <div key={i} style={{ padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6 }}>
                        <div style={{ fontFamily: M, fontSize: 14, fontWeight: 700, color: '#F5F5F5', marginBottom: 3 }}>{c.title || 'â€”'}</div>
                        <div style={{ fontFamily: M, fontSize: 14, color: RED, marginBottom: 4 }}>{c.role}{c.characterName ? ` as ${c.characterName}` : ''}{c.productionHouse ? ` Â· ${c.productionHouse}` : ''}</div>
                        ...
                        <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
                          {c.trailerLink && <a href={c.trailerLink} target="_blank" rel="noreferrer" style={{ fontFamily: M, fontSize: 13, color: RED, textDecoration: 'none' }}>â–¶ Trailer</a>}
                          {c.imdbLink && <a href={c.imdbLink} target="_blank" rel="noreferrer" style={{ fontFamily: M, fontSize: 13, color: GOLD, textDecoration: 'none' }}>IMDb â†’</a>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
                    {[
                      ['Profile Photo',  form.profilePhoto ? 'âœ“ Added' : 'âœ— Missing'],
                      ['Gallery Photos', galleryPhotos.length > 0 ? `âœ“ ${galleryPhotos.length} photos` : 'âœ— Missing'],
                      ['Showreel',       showreelUrl ? 'âœ“ Added' : 'âœ— Missing'],
                      ['Other Videos',   otherVideos.length > 0 ? `âœ“ ${otherVideos.length} videos` : 'Optional'],
                    ].map(([k, v]) => (
                      <div key={k} style={{ padding: '9px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6 }}>
                        <div style={{ fontFamily: M, fontSize: 14, color: '#6A7080', textTransform: 'uppercase' as const, letterSpacing: 1, marginBottom: 3 }}>{k}</div>
                        <div style={{ fontFamily: M, fontSize: 14, fontWeight: 600, color: (v as string).startsWith('âœ“') ? '#22c55e' : (v as string).startsWith('âœ—') ? '#ef4444' : '#A8B0BD' }}>{v}</div>
                      </div>
                    ))}
                  </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10 }}>
                <div style={{ display: 'flex', gap: 10 }}>
                  {activeSection > 1 && (
                    <button onClick={() => { setValidationError(''); setActiveSection(s => s - 1); }} style={{ padding: '9px 20px', background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 6, color: '#A8B0BD', fontFamily: B, fontSize: 14, letterSpacing: 1, cursor: 'pointer' }}>â† BACK</button>
                  )}
                  <button onClick={handleSaveDraft} style={{ padding: '9px 20px', background: 'transparent', border: `1px solid ${savedDraft ? '#22c55e' : 'rgba(255,255,255,0.15)'}`, borderRadius: 6, color: savedDraft ? '#22c55e' : '#A8B0BD', fontFamily: B, fontSize: 14, letterSpacing: 1, cursor: 'pointer', transition: 'all 0.2s' }}>
                    {savedDraft ? 'âœ“ SAVED' : 'SAVE DRAFT'}
                  </button>
                </div>
                {activeSection < 5
                  ? <button onClick={() => { if (validateSection(activeSection)) setActiveSection(s => s + 1); }} style={{ padding: '9px 32px', background: RED, border: 'none', borderRadius: 6, color: '#F5F5F5', fontFamily: B, fontSize: 17, letterSpacing: 1, cursor: 'pointer', boxShadow: '0 6px 20px rgba(200,32,42,0.3)' }}>NEXT STEP â†’</button>
                  : <button onClick={async () => {
                    ...
                    {isEditMode ? 'SAVE CHANGES â†’' : 'PUBLISH PROFILE â†’'}
                  </button>
                }
              </div>
            </div>
            ...
            <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column' as const, gap: 14 }}>
              ...
              <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span>ðŸ‘ï¸</span><span style={{ fontFamily: M, fontSize: 14, fontWeight: 700, color: '#F5F5F5' }}>Profile Preview</span></div>
                ...
                <div style={{ height: 180, background: form.profilePhoto ? 'transparent' : 'linear-gradient(160deg, #1a0a0a, #0B0F14)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  {form.profilePhoto ? <img src={form.profilePhoto} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : <span style={{ fontSize: 48, opacity: 0.2 }}>ðŸ‘¤</span>}
                </div>
                <div style={{ padding: '14px' }}>
                  <div style={{ fontFamily: M, fontSize: 14, fontWeight: 700, color: '#F5F5F5', marginBottom: 2 }}>{`${form.firstName} ${form.lastName}`.trim() || 'Your Name'} {(form.firstName || form.lastName) && <span style={{ color: '#22c55e', fontSize: 14 }}>âœ“</span>}</div>
                  ...
                  {form.height && <div style={{ fontFamily: M, fontSize: 14, color: '#6A7080', marginBottom: 3 }}>ðŸ“ {form.height} &nbsp; âš–ï¸ {form.weight}</div>}
                  {form.city && <div style={{ fontFamily: M, fontSize: 14, color: '#6A7080', marginBottom: 3 }}>ðŸ“ {form.city}{form.state ? `, ${form.state}` : ''}</div>}
                  {languages.length > 0 && <div style={{ fontFamily: M, fontSize: 14, color: '#6A7080', marginBottom: 10 }}>ðŸ—£ï¸ {languages.join(', ')}</div>}
              ...
              <div style={{ background: '#0B0F14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}><span>ðŸ’¡</span><span style={{ fontFamily: M, fontSize: 14, fontWeight: 700, color: '#F5F5F5' }}>Profile Tips</span></div>
                {[
                  { icon: 'ðŸ“¸', t: 'Add a clear profile photo',  d: 'Profiles with real photos get 70% more views.'           },
                  { icon: 'ðŸŽ¬', t: 'Upload a showreel',          d: 'A showreel increases your chances of getting noticed.'   },
                  { icon: 'âœ…', t: 'Complete all sections',      d: 'Complete profiles are 3x more likely to be shortlisted.' },
                  { icon: 'ðŸŽ­', t: 'Select correct departments', d: 'Agencies search by department and role. Be accurate.'    },
                ].map(tip => (
                ...
                <Link href="#" style={{ fontFamily: M, fontSize: 14, color: RED, textDecoration: 'none', fontWeight: 600 }}>View Success Stories â†’</Link>
              </div>
            </div>
          </div>
        </div>
