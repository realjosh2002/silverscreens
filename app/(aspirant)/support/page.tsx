...
const ROLES_DATA = [
  { id: 'superadmin',   name: 'Super Admin',       color: RED,    users: 0, description: 'Unrestricted access to all platform features including billing, system settings and role management.', system: true  },
  { id: 'admin',        name: 'Admin',              color: PURPLE, users: 0, description: 'Full platform access. Manages all modules, users, roles and system settings.',                        system: true  },
  { id: 'verifier',     name: 'Verifier',           color: BLUE,   users: 0, description: 'Reviews and verifies aspirant and agency profiles, KYC documents and media.',                         system: false },
  { id: 'content-mod',  name: 'Content Moderator',  color: GREEN,  users: 0, description: 'Manages CMS pages, banners, FAQs, notifications and advertisement content.',                          system: false },
  { id: 'support-agent',name: 'Support Agent',      color: TEAL,   users: 0, description: 'Handles support tickets, user complaints and platform inquiries.',                                    system: false },
];

const DEFAULT_PERMS: PermMatrix = initPerms(
  ROLES_DATA.map(function(r) { return r.id; }),
  ['superadmin', 'admin'],
  {
...
export default function RolesPermissionsPage() {
  ...
  const fetchAdminUsers = useCallback(async function() {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .select('id, name, email, role, is_active, last_login_at, profile_number, phone')
      .eq('role', 'admin')
    ...
    if (error) {
      ...
      // Update role user counts from real data
      const roleCounts: Record<string, number> = {};
      (data || []).forEach(function(u: any) { roleCounts[u.role] = (roleCounts[u.role] || 0) + 1; });
      setRoles(function(prev) {
        return prev.map(function(r) { return Object.assign({}, r, { users: roleCounts[r.id] || r.users }); });
      });
    }
    setUsersLoading(false);
  }, [showToast]);

  useEffect(function() { fetchAdminUsers(); }, [fetchAdminUsers]);
  ...
  function handleCreateRole() {
    ...
    const basePerms = newRoleBase !== 'Start from scratch'
      ? JSON.parse(JSON.stringify(perms[ROLES_DATA.find(function(r) { return r.name === newRoleBase; })?.id || 'admin'] || {}))
      : undefined;
    const newRole = {
      id: newId, name: newRoleName.trim(), color: TEAL, users: 0,
      description: newRoleDesc.trim() || 'Custom admin role.',
      system: false,
    };
    setRoles(function(prev) { return [...prev, newRole]; });
    if (basePerms) {
      setPerms(function(prev) { return Object.assign({}, prev, { [newId]: basePerms }); });
    } else {
      setPerms(function(prev) { return Object.assign({}, prev, { [newId]: initPerms([newId])[newId] }); });
    }
    setShowNewRole(false);
    setNewRoleName('');
    setNewRoleDesc('');
    setNewRoleBase('Start from scratch');
  ...
  const statCards = [
    { icon: '🛡️', label: 'Total Roles',       value: roles.length,                                                   color: PURPLE },
    { icon: '👥', label: 'Total Admin Users', value: adminUsers.length,                                               color: BLUE   },
    { icon: '✅', label: 'Active Users',      value: adminUsers.filter(function(u) { return u.is_active !== false; }).length, color: GREEN  },
    { icon: '🔒', label: 'System Roles',      value: roles.filter(function(r) { return r.system; }).length,           color: ORANGE },
  ];
        ...
        <div style={{ flex: 1, overflowY: 'auto' as const, padding: '18px 24px 32px', display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            {statCards.map(function(s) {
              return (
                <div key={s.label} style={{ flex: 1, ...cardStyle, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button
                            onClick={function() {
                              setConfirm({
                                ...
                                danger: true,
                                onConfirm: function() {
                                  setRoles(function(prev) { return prev.filter(function(r) { return r.id !== role.id; }); });
                                  setConfirm(null);
                        ...
                        <button
                          onClick={function() {
                            ...
                            setRoles(function(prev) { return [...prev, Object.assign({}, role, { id: dupId, name: dupName, system: false, users: 0 })]; });
                            setPerms(function(prev) { return Object.assign({}, prev, { [dupId]: JSON.parse(JSON.stringify(prev[role.id] || {})) }); });
                            showToast('Role "' + dupName + '" created.');
                          }}
