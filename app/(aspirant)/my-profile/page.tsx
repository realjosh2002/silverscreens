...
export default function MyProfilePage() {
  ...
  // ── Live data from API + localStorage ──
  const [liveUser, setLiveUser] = useState<{
    ...
    credits?: { type: string; year: string; role: string; title: string }[]
  }>({})
    ...
    <div style={{ display: 'flex', flexDirection: 'column' as const, height: '100vh', overflow: 'hidden', background: BG, fontFamily: BARLOW, color: '#F5F5F5' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <aside style={{ width: SB_W, flexShrink: 0, background: BG2, borderRight: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column' as const, overflowY: 'auto' as const, overflowX: 'hidden', scrollbarWidth: 'none' as const, transition: 'width 0.2s ease' }}>
          ...
          {/* Profile mini card when expanded */}
          {sidebarOpen && (
            <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
              <img src={liveUser.profile_image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=C8202A&color=fff`} alt={displayName} style={{ width: 38, height: 38, borderRadius: '50%', objectFit: 'cover', border: `2px solid ${GOLD}60`, flexShrink: 0 }} />
            ...
            <div style={{ position: 'relative' as const, zIndex: 1, display: 'flex', gap: 28, padding: '28px 28px 24px', alignItems: 'flex-start' }}>
              <div style={{ position: 'relative' as const, flexShrink: 0 }}>
                <div style={{ width: 170, height: 210, borderRadius: 10, overflow: 'hidden', border: '2px solid rgba(255,255,255,0.1)' }}>
                  <img src={liveUser.profile_image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=C8202A&color=fff&size=200`} alt={displayName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 22px', background: RED, border: 'none', borderRadius: 7, color: '#fff', fontFamily: BARLOW, fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(200,32,42,0.3)' }}>✉️ Message</button>
                  <button style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 22px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 7, color: '#F5F5F5', fontFamily: BARLOW, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>🔖 Shortlist</button>
                  <button style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 22px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 7, color: '#F5F5F5', fontFamily: BARLOW, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}><Share2 size={14} /> Share Profile</button>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
                  ...
                  <div style={{ background: BG2, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 20 }}>
                    ...
                    {displayPhotos.length === 0 ? (
                      <div style={{ textAlign: 'center' as const, padding: '30px 0', color: '#6A7080', fontSize: 15 }}>No photos uploaded yet.</div>
                    ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
                      {displayPhotos.map((p, i) => (
                        <div key={i} onClick={() => openLightbox(displayPhotos, i)} style={{ position: 'relative' as const, aspectRatio: '3/4', borderRadius: 8, overflow: 'hidden', border: `2px solid ${i === 0 ? GOLD : 'rgba(255,255,255,0.06)'}`, cursor: 'pointer', transition: 'transform 0.2s' }}
                          onMouseEnter={e => (e.currentTarget.style.transform = 'scale(1.03)')}
                          onMouseLeave={e => (e.currentTarget.style.transform = 'scale(1)')}>
                          <img src={p} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          {i === 0 && <div style={{ position: 'absolute' as const, top: 6, left: 6, background: GOLD, color: '#000', fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4 }}>PRIMARY</div>}
                        </div>
                      ))}
                    </div>
                    )}
                  ...
                  <div style={{ background: BG2, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 20 }}>
                    ...
                    {displayVideos.length === 0 ? (
                      <div style={{ textAlign: 'center' as const, padding: '30px 0', color: '#6A7080', fontSize: 15 }}>No videos uploaded yet.</div>
                    ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                      {displayVideos.map((v, i) => (
                        <div key={i} onClick={() => setVideoPlayer(v.url)} style={{ position: 'relative' as const, borderRadius: 8, overflow: 'hidden', aspectRatio: '16/9', cursor: 'pointer', transition: 'transform 0.2s' }}
                          onMouseEnter={e => (e.currentTarget.style.transform = 'scale(1.02)')}
                          onMouseLeave={e => (e.currentTarget.style.transform = 'scale(1)')}>
                          <video src={v.url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          ...
                          <div style={{ position: 'absolute' as const, bottom: 8, left: 10, fontSize: 13, color: '#fff', background: 'rgba(0,0,0,0.65)', padding: '2px 7px', borderRadius: 4 }}>{v.title}</div>
                        </div>
                      ))}
                    </div>
                    )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
                  {displayCredits.filter(exp => exp.title || exp.role).length > 0 ? displayCredits.filter(exp => exp.title || exp.role).map((exp, i) => (
                    <div key={i} style={{ background: BG2, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 20 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: exp.description ? 12 : 0 }}>
                        <div>
                      ...
                      {exp.description && <p style={{ fontSize: 15, color: '#A8B0BD', lineHeight: 1.6, margin: 0 }}>{exp.description}</p>}
                    </div>
                  )) : (
              ...
              {tab === 'Documents' && (
                <div style={{ background: BG2, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    ...
                    <button onClick={() => router.push('/settings')} style={{ padding: '6px 16px', background: RED, border: 'none', borderRadius: 6, color: '#fff', fontFamily: BARLOW, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>+ Upload</button>
                  </div>
      ...
      {lightboxPhoto && (
        <div onClick={() => setLightboxPhoto(null)}
          ...
          <div onClick={e => e.stopPropagation()} style={{ maxWidth: '80vw', maxHeight: '85vh', position: 'relative' as const }}>
            <img src={lightboxPhoto} alt="" style={{ maxWidth: '80vw', maxHeight: '85vh', borderRadius: 10, objectFit: 'contain', display: 'block', boxShadow: '0 20px 80px rgba(0,0,0,0.8)' }} />
            {/* Counter */}
        ...
        <div onClick={() => setVideoPlayer(null)}
          ...
          <div onClick={e => e.stopPropagation()} style={{ width: '75vw', maxWidth: 960 }}>
            {videoPlayer.startsWith('http') ? (
              <video src={videoPlayer} controls autoPlay style={{ width: '100%', borderRadius: 10, outline: 'none', boxShadow: '0 20px 80px rgba(0,0,0,0.8)' }} />
            ) : (
