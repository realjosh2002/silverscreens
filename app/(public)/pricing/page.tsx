...
/* ─────────────────────────────────────────────────────────────
   PLANS — fetched dynamically from /api/plans
   Admin changes to plans reflect here automatically.
   The hardcoded arrays below are FALLBACKS only, used if the
   API is unreachable (e.g. during SSR or network error).
───────────────────────────────────────────────────────────── */
...
function apiPlanToShape(p: any, fallbackFeatures?: PlanFeature[]): Plan {
  const months = p.duration_months ?? 1
  const price  = Number(p.price ?? 0)
  const features: PlanFeature[] = Array.isArray(p.features)
    ? p.features.map((f: any) => typeof f === 'string' ? { text: f, included: true } : f)
    : (p.features?.included ?? []).map((f: string) => ({ text: f, included: true })).concat(
        (p.features?.excluded ?? []).map((f: string) => ({ text: f, included: false }))
      )
  const iconMap: Record<string, string> = { spotlight: 'zap', star: 'star', icon: 'crown', starter: 'building', growth: 'trending', enterprise: 'layers' }
  const key = (p.plan_key ?? p.id ?? '').toLowerCase()
  return {
    id:              key,
    name:            p.plan_name ?? p.name ?? key,
    icon:            iconMap[key] ?? (p.user_type === 'agency' ? 'building' : 'star'),
    duration:        months === 1 ? '1 Month' : months === 3 ? '3 Months' : months === 6 ? '6 Months' : months === 12 ? '12 Months' : `${months} Months`,
    durationShort:   `${months} mo`,
    months,
    price,
    pricePerMonth:   price / months,
    usdPrice:        Math.round(price / 84),
    usdPricePerMonth:Math.round((price / months) / 84 * 100) / 100,
    tagline:         p.tagline ?? p.description ?? '',
    popular:         p.is_featured ?? false,
    badge:           p.is_featured ? 'Most Popular' : (months === 12 ? 'Best Value' : null),
    features:        features.length > 0 ? features : (fallbackFeatures ?? []),
  }
}

/* Fallback plans used only if API is unreachable */
const FALLBACK_ASPIRANT_PLANS: Plan[] = [
  {
    id: 'spotlight', name: 'Spotlight', icon: 'zap',
    duration: '3 Months', durationShort: '3 mo', months: 3,
    price: 299, pricePerMonth: 99.67, usdPrice: 3, usdPricePerMonth: 1,
  ...
  },
  {
    ...
    price: 499, pricePerMonth: 83.17, usdPrice: 5, usdPricePerMonth: 0.83,
  ...
  },
  {
    ...
    price: 999, pricePerMonth: 83.25, usdPrice: 10, usdPricePerMonth: 0.83,
...
const FALLBACK_AGENCY_PLANS: Plan[] = [
  {
    id: 'starter', name: 'Starter', icon: 'building',
    duration: '3 Months', durationShort: '3 mo', months: 3,
    price: 5999, pricePerMonth: 1999.67, usdPrice: 63, usdPricePerMonth: 21,
  ...
  },
  {
    ...
    duration: '6 Months', durationShort: '6 mo', months: 6,
    price: 24999, pricePerMonth: 4166.5, usdPrice: 263, usdPricePerMonth: 43.83,
  ...
  },
  {
    ...
    duration: '12 Months', durationShort: '12 mo', months: 12,
    price: 99999, pricePerMonth: 8333.25, usdPrice: 1050, usdPricePerMonth: 87.50,
...
function useAuth() {
  ...
  useEffect(() => {
    try {
      ...
      setUser({ loggedIn: true, name: parsed.name ?? '', category: parsed.category ?? parsed.userType ?? '' })
    } catch {
...
/* ── AGENCY PLAN CARD ────────────────────────────────────────── */
function AgencyCard({ plan, onSelect, isGuest, currency }: {
  plan: Plan
  isGuest: boolean
  currency: 'INR' | 'USD'
...
function PricingInner() {
  ...
  // Plans fetched dynamically from API — reflects admin changes immediately
  const [aspirantPlans, setAspirantPlans] = useState<Plan[]>(FALLBACK_ASPIRANT_PLANS)
  const [agencyPlans,   setAgencyPlans]   = useState<Plan[]>(FALLBACK_AGENCY_PLANS)
  const [plansLoaded,   setPlansLoaded]   = useState(false)

  useEffect(() => {
    try {
      // Redirect non-India users to international pricing
      const u = JSON.parse(localStorage.getItem('ss_user') || '{}')
      if (u?.country && u.country.toLowerCase() !== 'india') {
        router.replace('/pricing-international')
        return
      }
    } catch {}
    ...
    fetch('/api/plans')
      ...
      .then(data => {
        if (!data) return
        const all = data.data?.all_plans ?? data.all_plans ?? []
        const aspirant = all.filter((p: any) => p.user_type === 'aspirant').map((p: any) => apiPlanToShape(p))
        const agency   = all.filter((p: any) => p.user_type === 'agency').map((p: any) => apiPlanToShape(p))
      ...
      .catch(() => { setPlansLoaded(true) }) // keep fallbacks on error
  }, [])
      ...
      <div style={{ position: 'relative', overflow: 'hidden', minHeight: 420, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center' as const, padding: '100px 80px 60px' }}>
          ...
          <h1 style={{ fontFamily: BEBAS, fontSize: 'clamp(52px, 7vw, 88px)', letterSpacing: 6, color: '#F5F5F5', marginBottom: 16, lineHeight: 0.9, textShadow: '0 0 80px rgba(200,32,42,0.25)' }}>
            PRICING<br /><span style={{ color: RED }}>PLANS</span>
          </h1>
          <p style={{ fontFamily: BARLOW, fontSize: 19, color: 'rgba(255,255,255,0.5)', fontStyle: 'italic', letterSpacing: 0.3, marginBottom: 12 }}>
            One platform. Endless opportunity. Choose the plan that launches your career.
          </p>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: '6px 16px', fontSize: 14, fontFamily: BARLOW, color: 'rgba(255,255,255,0.4)' }}>
            🇮🇳 India Pricing (₹) &nbsp;·&nbsp;
            <span onClick={() => router.push('/pricing-international')} style={{ color: GOLD, cursor: 'pointer', textDecoration: 'underline' }}>International Pricing (USD) →</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'start' }}>
            {tab === 'aspirants'
              ...
              : agencyPlans.map(plan => (
                <AgencyCard key={plan.id} plan={plan} isGuest={isGuest} onSelect={handleAgencySelect} currency='INR' />
              ))
            }
          </div>
