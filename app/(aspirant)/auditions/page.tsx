  ...
  useEffect(() => {
    async function fetchAuditions() {
      try {
        ...
        const res = await fetch('/api/auditions?limit=100', {
        ...
        // API failed — show empty state, not dummy data
        setAuditions([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: BG, fontFamily: BARLOW, color: '#fff' }}>
      ...
      <header style={{
        ...
        <div onClick={() => router.push('/messages')} style={{ width: 36, height: 36, borderRadius: 8, background: 'rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
          ...
          <main style={{ flex: 1, padding: '20px 16px 20px 20px', minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.08)', marginBottom: 16 }}>
              ...
              <div ref={sortRef} style={{ position: 'relative' }}>
                <button onClick={() => setSortOpen(v => !v)} style={{
                  ...
                  padding: '7px 12px', fontSize: 14, fontFamily: BARLOW, cursor: 'pointer',
                }}>
                  {sortBy} <ChevronDown size={13} color="rgba(255,255,255,0.4)" style={{ transform: sortOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }} />
                </button>
                {sortOpen && (
                  <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, width: 180, background: BG3, border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, overflow: 'hidden', zIndex: 100, boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', gap: 16 }}>
                <div style={{ width: 36, height: 36, border: `3px solid rgba(255,255,255,0.1)`, borderTopColor: RED, borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
                <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.4)' }}>Loading your auditions…</div>
            ...
            {!loading && filtered.length === 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', gap: 16, textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: BEBAS, fontSize: 24, letterSpacing: 1, color: '#fff' }}>No Auditions Yet</div>
                <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.45)', maxWidth: 380, lineHeight: 1.6 }}>
                  You haven't been invited to any auditions yet. Apply to casting calls and agencies will invite you for auditions once they shortlist you.
                </div>
                <button onClick={() => router.push('/casting-calls')} style={{ marginTop: 8, padding: '10px 24px', background: RED, border: 'none', borderRadius: 8, color: '#fff', fontFamily: BARLOW, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'stretch' }}>
                        ...
                        <div style={{ flex: 1, padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                            <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)' }}>{a.type}{a.genres ? ` • ${a.genres}` : ''}</span>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginTop: 2 }}>
                            <div style={{ display: 'flex', gap: 24 }}>
                              ...
                              <div>
                                <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.35)', marginBottom: 1 }}>Mode</div>
                                <div style={{ fontSize: 15, fontWeight: 600, color: '#fff' }}>{a.mode === 'online' ? 'Virtual' : a.mode === 'both' ? 'Hybrid' : 'In-Person'}</div>
                              </div>
                            </div>
                      ...
                      {isExpanded && (
                        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', background: BG3, padding: '12px 16px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px 16px' }}>
                          {[
                            { label: 'Audition Type', value: a.mode === 'online' ? 'Virtual' : a.mode === 'both' ? 'Hybrid' : 'In-Person' },
          ...
          <div style={{ width: 360, flexShrink: 0, display: 'flex' }}>
            <div style={{ flex: 1, padding: '20px 16px 20px 4px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ background: BG2, border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '16px' }}>
                ...
                {auditions.filter(a => a.status === 'Upcoming').length === 0 ? (
                  <div style={{ padding: '16px 0', textAlign: 'center', fontSize: 14, color: 'rgba(255,255,255,0.3)' }}>No upcoming auditions</div>
                ) : (
