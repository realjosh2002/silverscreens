  ...
  useEffect(() => {
    ...
    fetch('/api/profile/agency', { headers })
      ...
      .then(d => {
        ...
        // Parse established year from created_at as fallback; agency stores it via years_of_experience
        const estYear = p.established_year
          ?? (p.created_at ? String(new Date(p.created_at).getFullYear()) : '')
        ...
        if (p.logo_url)   setLogoSrc(p.logo_url)
        if (p.banner_url) setBannerSrc(p.banner_url)
      }).catch(() => {})
