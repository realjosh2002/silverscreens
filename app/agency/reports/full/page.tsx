          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <h1 style={{ fontFamily: BEBAS, fontSize: 28, color: GOLD, margin: 0, letterSpacing: 1 }}>FULL CASTING REPORT</h1>
              </div>
            </div>
            <button style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'transparent', border: `1px solid ${GOLD}`, borderRadius: 7, color: GOLD, fontFamily: BARLOW, fontSize: 14, padding: '8px 16px', cursor: 'pointer' }}>
              ⬇ Export Report
            </button>
          </div>

          {statsLoading ? (
            <div style={{ textAlign: 'center', padding: '60px 0', color: GRAY, fontSize: 16 }}>Loading report data…</div>
          ) : (
            <>
              {/* Summary Cards */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
                {[
                  { label: 'Total Castings',    value: apiStats?.casting_calls?.total ?? 0,    color: GOLD  },
                  { label: 'Active',             value: apiStats?.casting_calls?.active ?? 0,   color: GREEN },
                  { label: 'Total Applicants',   value: total,                                   color: WHITE },
                  { label: 'Shortlisted',        value: apiStats?.applicants?.shortlisted ?? 0, color: GOLD  },
                  { label: 'Auditions Done',     value: apiStats?.auditions?.completed ?? 0,    color: GREEN },
                  { label: 'Selected',           value: apiStats?.applicants?.selected ?? 0,    color: GREEN },
                  { label: 'Rejected',           value: apiStats?.applicants?.rejected ?? 0,    color: RED   },
                  { label: 'Total Views',        value: apiStats?.casting_views ?? 0,           color: WHITE },
                ].map(card => (
                  <div key={card.label} style={{ background: BG3, border: `1px solid ${BG4}`, borderRadius: 10, padding: '14px 16px' }}>
                    <div style={{ fontSize: 13, color: GRAY, marginBottom: 6 }}>{card.label}</div>
                    <div style={{ fontFamily: BEBAS, fontSize: 26, color: card.color, lineHeight: 1 }}>{typeof card.value === 'number' ? card.value.toLocaleString() : card.value}</div>
                  </div>
                ))}
              </div>

              {/* Application Funnel */}
              <div style={{ background: BG3, border: `1px solid ${BG4}`, borderRadius: 12, padding: '20px 24px', marginBottom: 20 }}>
                <div style={{ fontFamily: BEBAS, fontSize: 18, color: WHITE, marginBottom: 16, letterSpacing: 1 }}>APPLICATION FUNNEL</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[
                    { label: 'Applied',     value: apiStats?.applicants?.applied ?? 0,     color: '#3B82F6' },
                    { label: 'In Review',   value: apiStats?.applicants?.in_review ?? 0,   color: GOLD      },
                    { label: 'Shortlisted', value: apiStats?.applicants?.shortlisted ?? 0, color: '#A78BFA' },
                    { label: 'Auditioned',  value: apiStats?.auditions?.completed ?? 0,    color: GREEN     },
                    { label: 'Selected',    value: apiStats?.applicants?.selected ?? 0,    color: GREEN     },
                    { label: 'Rejected',    value: apiStats?.applicants?.rejected ?? 0,    color: RED       },
                  ].map(row => {
                    ...
                    return (
                      <div key={row.label}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                          <span style={{ fontSize: 14, color: LIGHT }}>{row.label}</span>
                          <span style={{ fontSize: 14, color: WHITE, fontWeight: 700 }}>{row.value.toLocaleString()} <span style={{ color: GRAY, fontWeight: 400 }}>({pct}%)</span></span>
                        </div>
                        <div style={{ height: 6, background: BG4, borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{ width: `${pct}%`, height: '100%', background: row.color, borderRadius: 3, transition: 'width 0.5s ease' }} />
                        </div>
                      </div>
                    );
                  })}
              ...
              {/* Casting Calls Table */}
              <div style={{ background: BG3, border: `1px solid ${BG4}`, borderRadius: 12, overflow: 'hidden' }}>
                <div style={{ padding: '16px 24px', borderBottom: `1px solid ${BG4}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontFamily: BEBAS, fontSize: 18, color: WHITE, letterSpacing: 1 }}>CASTING CALL PERFORMANCE</div>
                  <span style={{ fontSize: 13, color: GRAY }}>Top {(apiStats?.top_casting_calls ?? []).length} by applications</span>
                </div>
                {(apiStats?.top_casting_calls ?? []).length === 0 ? (
                  <div style={{ padding: '40px 24px', textAlign: 'center', color: GRAY, fontSize: 15 }}>No casting calls yet.</div>
                ) : (
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ background: BG4 }}>
                          {['Casting Call', 'Status', 'Applications', 'Views', 'Deadline'].map(h => (
                            <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: GRAY, fontFamily: BARLOW, whiteSpace: 'nowrap' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {(apiStats?.top_casting_calls ?? []).map((cc: any) => (
                          <tr key={cc.id}
                            onClick={() => router.push(`/agency/casting-calls/${cc.id}`)}
                            style={{ borderTop: `1px solid ${BG4}`, cursor: 'pointer' }}
                            onMouseEnter={e => (e.currentTarget.style.background = BG4)}
                            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                          >
                            <td style={{ padding: '12px 16px', fontSize: 15, color: WHITE, fontWeight: 600, maxWidth: 260 }}>
                              <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cc.title}</div>
                            </td>
                            <td style={{ padding: '12px 16px' }}>
                              <span style={{ background: cc.status === 'active' ? 'rgba(34,197,94,0.15)' : 'rgba(255,255,255,0.07)', color: cc.status === 'active' ? GREEN : GRAY, borderRadius: 6, padding: '2px 10px', fontSize: 12, fontWeight: 700, textTransform: 'capitalize', fontFamily: BARLOW }}>{cc.status}</span>
                            </td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: WHITE, fontWeight: 700 }}>{(cc.applications_count ?? 0).toLocaleString()}</td>
                            <td style={{ padding: '12px 16px', fontSize: 15, color: LIGHT }}>{(cc.views_count ?? 0).toLocaleString()}</td>
                            <td style={{ padding: '12px 16px', fontSize: 13, color: GRAY, whiteSpace: 'nowrap' }}>
                              {cc.last_application_date ? new Date(cc.last_application_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
